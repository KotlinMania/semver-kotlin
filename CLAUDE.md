# Claude Code Project Instructions — semver-kotlin

## Project Overview

This is **semver-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`semver`](https://crates.io/crates/semver).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/semver/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/semver>

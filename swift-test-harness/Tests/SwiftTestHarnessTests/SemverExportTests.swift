...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Semver.swiftmodule/`
...
//      `import Semver` above would fail at compile time.
//
//   2. The static archive `libSemver.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Semver" }` and
//      `flattenPackage = "io.github.kotlinmania.semver"` configuration in
...
final class SemverExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Semver swift module imported cleanly")
    }
}
